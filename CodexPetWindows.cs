...
static class CrashLog {
    ...
    public static void WriteMessage(string message){try{File.AppendAllText(PathName,"\r\n["+DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")+"]\r\n"+message+"\r\n",Encoding.UTF8);}catch{}}
}
static class Native {
...
sealed class SettingsForm:Form {
    ...
    public readonly ComboBox BookMode=new ComboBox(),PetTheme=new ComboBox();
    public string ThemeId { get { return PetTheme.SelectedIndex==1?"newtheme":"codex"; } }
    public SettingsForm(int idle,int pulse,int dailyNew,int autoWordMinutes,int waterMinutes,int snooze,int petScale,bool awake,bool autoSpeak,bool autoWordEnabled,bool pauseFullscreen,bool waterEnabled,bool top,bool eyeFollow,bool autoStart,string book,string themeId){
        ...
        Combo(petTab,"外观主题",PetTheme,22,24,new string[]{"Codex 蓝色","新增主题"},themeId=="newtheme"?"新增主题":"Codex 蓝色");Num(petTab,"宠物大小（百分比）",PetScale,22,70,60,180,10,petScale);Check(Top,"始终置顶",22,122,top,petTab);Check(EyeFollow,"鼠标靠近时跟随视线",250,122,eyeFollow,petTab);Info(petTab,"保存后立即切换主题。单击互动，双击背词，按住拖动；宠物本身不提供右键菜单。",22,171,430);
...
sealed class PetForm:Form {
    ...
    Image atlas;readonly List<Word> ielts=new List<Word>(),daily=new List<Word>();DateTime realInput=DateTime.UtcNow,lastPulse=DateTime.UtcNow,nextWater,nextAutoWord,statusUntil,actionUntil;uint lastInputTick;string action="idle",footStatus="",themeId="codex";int frame,actionFrame,idleMin,pulseMin,dailyNew,autoWordMinutes,waterMinutes,snoozeMinutes,petScale,petSide;bool awake,locked,warned,dragging,top,eyeFollow,autoSpeak,autoWordEnabled,pauseFullscreen,waterEnabled,pendingWater,pendingWord;Point dragAt;WordCardForm wordCard;WaterForm waterCard;NoticeForm lockCard;
    public PetForm(){LoadSettings();Text="Codex 桌宠";FormBorderStyle=FormBorderStyle.None;ShowInTaskbar=false;BackColor=Color.Fuchsia;TransparencyKey=Color.Fuchsia;DoubleBuffered=true;StartPosition=FormStartPosition.Manual;ApplySize();Location=Visible(new Point(Read("X",Screen.PrimaryScreen.WorkingArea.Right-Width,-10000,10000),Read("Y",Screen.PrimaryScreen.WorkingArea.Bottom-Height,-10000,10000)));ApplyTheme(themeId,true);LoadWords();BuildTray();
    ...
    void OpenSettings(){
        using(SettingsForm f=new SettingsForm(idleMin,pulseMin,dailyNew,autoWordMinutes,waterMinutes,snoozeMinutes,petScale,awake,autoSpeak,autoWordEnabled,pauseFullscreen,waterEnabled,top,eyeFollow,AutoStart(),ReadString("BookMode","全部词库"),themeId))if(f.ShowDialog()==DialogResult.OK){
            idleMin=(int)f.Idle.Value;pulseMin=(int)f.Pulse.Value;dailyNew=(int)f.DailyNew.Value;autoWordMinutes=(int)f.AutoWordMinutes.Value;waterMinutes=(int)f.WaterMinutes.Value;snoozeMinutes=(int)f.Snooze.Value;petScale=(int)f.PetScale.Value;awake=f.Awake.Checked;autoSpeak=f.AutoSpeak.Checked;autoWordEnabled=f.AutoWordEnabled.Checked;pauseFullscreen=f.PauseFullscreen.Checked;waterEnabled=f.WaterEnabled.Checked;top=f.Top.Checked;eyeFollow=f.EyeFollow.Checked;TopMost=top;
            Write("Idle",idleMin);Write("Pulse",pulseMin);Write("DailyNew",dailyNew);Write("AutoWordMinutes",autoWordMinutes);Write("WaterMinutes",waterMinutes);Write("Snooze",snoozeMinutes);Write("PetScale",petScale);Write("Awake",awake?1:0);Write("AutoSpeak",autoSpeak?1:0);Write("AutoWordEnabled",autoWordEnabled?1:0);Write("PauseFullscreen",pauseFullscreen?1:0);Write("WaterEnabled",waterEnabled?1:0);Write("Top",top?1:0);Write("EyeFollow",eyeFollow?1:0);WriteString("BookMode",f.BookMode.SelectedItem.ToString());SetAuto(f.AutoStartBox.Checked);
            ApplyTheme(f.ThemeId,true);ApplySize();Location=Visible(Location);nextWater=DateTime.Now.AddMinutes(waterMinutes);nextAutoWord=DateTime.Now.AddMinutes(autoWordMinutes);realInput=lastPulse=DateTime.UtcNow;BuildTray();Invalidate();
        }
    }
    ...
    void ApplyTheme(string requested,bool notify){
        string id=requested=="newtheme"?"newtheme":"codex",resource=id=="newtheme"?"CodexPet.newtheme.atlas.png":"CodexPet.atlas.png";Image next=null;
        try{next=Load(resource);if(next==null)throw new FileNotFoundException("找不到内嵌主题资源："+resource);SwapAtlas(next);next=null;themeId=id;WriteString("ThemeId",themeId);return;}
        catch(Exception ex){if(next!=null)next.Dispose();CrashLog.WriteMessage("主题加载失败（"+id+"）："+ex);}
        if(id!="codex"){
            try{next=Load("CodexPet.atlas.png");if(next==null)throw new FileNotFoundException("找不到默认主题资源：CodexPet.atlas.png");SwapAtlas(next);next=null;themeId="codex";WriteString("ThemeId",themeId);if(notify)MessageBox.Show("所选主题资源不可用，已安全切回 Codex 蓝色。\r\n详细信息已记录到：\r\n"+CrashLog.PathName,"主题加载失败",MessageBoxButtons.OK,MessageBoxIcon.Warning);return;}
            catch(Exception ex){if(next!=null)next.Dispose();CrashLog.WriteMessage("默认主题回退失败："+ex);}
        }
        if(notify)MessageBox.Show("主题资源无法加载，程序将保留当前外观。\r\n详细信息已记录到：\r\n"+CrashLog.PathName,"主题加载失败",MessageBoxButtons.OK,MessageBoxIcon.Error);
    }
    void SwapAtlas(Image next){Image old=atlas;atlas=next;if(old!=null)old.Dispose();Invalidate();}
    new Image Load(string name){Stream s=Assembly.GetExecutingAssembly().GetManifestResourceStream(name);if(s==null)return null;using(s)using(Image raw=Image.FromStream(s)){if(raw.Width!=1536||raw.Height!=2288)throw new InvalidDataException("主题图集尺寸错误："+raw.Width+"x"+raw.Height+"，应为 1536x2288");Bitmap b=new Bitmap(raw.Width,raw.Height,PixelFormat.Format32bppArgb);using(Graphics g=Graphics.FromImage(b)){g.CompositingMode=CompositingMode.SourceCopy;g.DrawImage(raw,0,0);}return b;}}
